...
The comments in this codebase are unusually good: they explain *why*, cite
...
| C5 | `routes/problems.ts:35` `toCandidateProblem` → reuse `lib/candidatePool.ts:57` `toPoolCandidate` | 2 sites | Only 2 sites, but it is the *same* JSON parser with one a strict subset of the other — "same logic", not "similar-looking". |
...
4. **`apps/web/e2e/smoke.spec.ts` is stale and cannot pass.** It navigates to `/baseline`
   ...
   **needs your eyes.**
5. **Python `mypy` failures (7).** Includes two *unused* `# type: ignore` comments
...
- `hast-util-sanitize` — knip "unused dependency"; `rehype-sanitize` re-exports
  `defaultSchema` from it (`Markdown.tsx:11`), and declaring it directly is *correct*

...
describe.skipIf(pool === null)("runWorker (integration, real Postgres)", () => {
  ...
  it("8. end-to-end: N jobs, concurrency 4, all acked, none double-processed", async () => {
  ...
  it("9. worker-kill recovery: hung handler + expired lease -> reaper requeues -> second worker completes exactly once", async () => {
    const queue = new Queue(p, { leaseSeconds: 30 });

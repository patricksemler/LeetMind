/**
 * Static C++ harness text spliced by `generateMainCpp` (./codegen.ts) into every generated
 * `main.cpp`, per CONTRACTS §7. Kept in its own module — rather than inline in codegen.ts — so
 * this large block of hand-written C++ (structs, codecs, comparators, the per-test driver) reads
 * as C++ text sitting alongside the codegen logic, instead of being buried inside a TS module
 * dominated by string-template plumbing.
 */

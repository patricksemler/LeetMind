...
describe.skipIf(!dbReachable)("chaos 5: concurrent claim storm (many workers, many jobs, real DB contention)", () => {
  const jobIds: string[] = [];
  ...
  it("50 jobs, 10 concurrent claimers: every job processed exactly once, none lost, none duplicated", async () => {
    ...
    await Promise.all(
      Array.from({ length: CLAIMER_COUNT }, (_, i) => claimLoop(queue, `storm-worker-${i}`, processed[i]!)),
    );

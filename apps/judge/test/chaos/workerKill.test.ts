...
describe.skipIf(!canRun)(
  "chaos 2: worker killed mid-judge (real SIGKILL of a real subprocess)",
  () => {
    ...
    it(`SIGKILL mid-execution -> reaper requeues -> a second worker completes exactly once, recovery < ${RECOVERY_BUDGET_MS}ms`, async () => {
      ...
      await waitFor(
        ...
        },
        {
          timeoutMs: 8000,
          intervalMs: 20,
          describe: `submission ${submission.id} reaching status='running'`,
        },
      );
      ...
      console.log(
        `[chaos 2] worker-kill recovery time: ${recoveryMs}ms (budget ${RECOVERY_BUDGET_MS}ms)`,
      );

...
describe.skipIf(!dbReachable)("chaos 7: reaper idempotence (concurrent reapers, real DB contention)", () => {
  const jobIds: string[] = [];
  ...
  it("N jobs with expired leases, 5 concurrent reapers: every job reaped exactly once, none left leased", async () => {
    ...
    for (let i = 0; i < JOB_COUNT; i++) {
      const job = await queue.enqueue(pool, { kind: CHAOS_QUEUE_KIND, payload: { i }, maxAttempts: 3 });
      ids.push(job!.id);
    ...
    await pool.query(`update jobs set lease_expires_at = now() - interval '1 second' where kind = $1`, [CHAOS_QUEUE_KIND]);

    const REAPER_COUNT = 5;
    const reapers = Array.from({ length: REAPER_COUNT }, () => new Queue(pool, { workerId: `reaper-${Math.random()}` }));
    ...
    while (true) {
      ...
      if (!job) break;
      expect(job.attempts, `job ${job.id} should have attempts=2 (claimed once, reaped, reclaimed once) — got ${job.attempts}`).toBe(2);

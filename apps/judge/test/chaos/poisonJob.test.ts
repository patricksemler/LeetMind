...
describe.skipIf(!dbReachable)("chaos 4: poison job (always-throwing handler -> dead after max_attempts, never loops forever)", () => {
  const jobIds: string[] = [];
  ...
  it("lands in 'dead' after exactly max_attempts, records the last thrown error, and stops being claimable", async () => {
    ...
    expect(dead.attempts, "must have stopped at exactly max_attempts, not kept retrying").toBe(maxAttempts);
    expect(dead.max_attempts).toBe(maxAttempts);
    expect(dead.last_error).toContain("poison job handler call");
    expect(handlerCalls, "the handler must have been invoked exactly max_attempts times, never more").toBe(maxAttempts);

...
describe.skipIf(!canRun)(
  "chaos 3: lease theft (real reaper-driven, not a mocked heartbeat)",
  () => {
    ...
    it("a genuinely stolen lease makes heartbeat() return false; handler aborts before writing a verdict", async () => {
      ...
      await waitFor(
        ...
        },
        {
          timeoutMs: 10_000,
          intervalMs: 100,
          describe: `reaper requeuing worker-A's expired lease on job ${jobId}`,
        },
      );
      ...
      expect(
        heartbeatCalls,
        "expected the handler to actually call ctx.heartbeat() before executing",
      ).toBeGreaterThan(0);

      const reloaded = await reloadSubmission(submission.id);
      expect(
        reloaded.status,
        "submission must still be non-terminal — worker A must not write a verdict it no longer owns",
      ).not.toBe("completed");

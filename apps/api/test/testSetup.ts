...
// `pnpm --filter @leetmind/api test` concurrently with `pnpm --filter @leetmind/judge test` (or
// with content's `pytest`) against the same `leetmind_test` database no longer collides — each
...
// finish so `leetmind_test` doesn't accumulate schemas across repeated runs.
afterAll(async () => {

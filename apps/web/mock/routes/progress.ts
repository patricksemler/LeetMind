...
export function registerProgressRoutes(app: Express): void {
  ...
  app.get(
    ...
    handle((_req, res) => {
      ...
      const submissionRows = [...submissions.values()].filter(
        (s) => s.mode === "submit" && s.row.status === "completed",
      );
      const byBand = new Map<
        number,
        { solved_without_hints: number; solved_with_hints: number; attempts: number }
      >();
      ...
      const activeMsSamples = submissionRows
        .map((s) => s.activeMs)
        .filter((n) => n > 0)
        .sort((a, b) => a - b);
      ...
      for (const cs of conceptState.values()) {
        for (const [k, v] of Object.entries(cs.error_counts))
          errorCounts.set(k, (errorCounts.get(k) ?? 0) + v);
      }
      ...
      const highestUnassisted = problemFixtures
        .filter(
          (p) =>
            getProblemUserState(p.problemVersionId).solved &&
            getProblemUserState(p.problemVersionId).hintsTaken.length === 0,
        )
      ...
      res.json({
        ...
        records: {
          highest_unassisted_difficulty_solved:
            highestUnassisted?.content.difficulty.rating ?? null,
  ...
  app.get(
    ...
    handle((_req, res) => {
      ...
      for (const s of submissions.values()) {
        if (s.row.verdict)
          verdictCounts.set(s.row.verdict, (verdictCounts.get(s.row.verdict) ?? 0) + 1);
      }
      ...
      res.json({
        queue: {
          kinds: [
            ...
            {
              kind: "generate",
              counts: { queued: jitter() },
              oldest_queued_age_ms: jitter() * 1000,
            },
          ],
        ...
        workers: [
          {
            worker_id: "judge-mock-1",
            kind: "judge",
            last_seen_at: new Date().toISOString(),
            stale: false,
          },
          {
            worker_id: "content-mock-1",
            kind: "content",
        ...
        verdicts: {
          window: "24h",
          counts: [...verdictCounts.entries()].map(([verdict, count]) => ({ verdict, count })),
        },

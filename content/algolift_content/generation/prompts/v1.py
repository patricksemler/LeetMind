...
(a) parses as `algolift_content.models.ProblemVersion` and (b) has a real chance of surviving the
...
inside the prompt. Two consumers depend on this block being present and well-formed:
  ...
  2. `algolift_content.generation.invoker.StubInvoker`, which regexes this block back out of the
...
REQUEST_JSON_BEGIN = "<<<ALGOLIFT_GENERATION_REQUEST_JSON"
REQUEST_JSON_END = "ALGOLIFT_GENERATION_REQUEST_JSON>>>"
...
def build_generation_prompt(request: GenerationRequest) -> str:
    ...
    matching `algolift_content.models.ProblemVersion`, nothing else — no prose before or after,

...
Layout:
    ...
    envelope.py     — the ALGOLIFT envelope wire format: `parse_envelope(text) -> dict`,
    ...
    prompts/v2.py   — `PROMPT_VERSION = "v2"` (the default), the ALGOLIFT-envelope prompt.
    ...
    handler.py      — `handle_generate(job, ctx)`, the `'generate'` job handler. Import path for
                       ...
                       `algolift_content.generation.handler.handle_generate`.

Every model invocation (success, schema failure, or invoker/infrastructure failure) writes an
`algolift_content.models.ModelRun` row — this powers the cost-per-approved-problem metric and

"""The ALGOLIFT envelope — a delimited wire format for generation responses that needs no string
...
(`<<<ALGOLIFT_RESULT>>>` + one trailing JSON object) and documented in CONTRACTS.md §6:

    <<<ALGOLIFT_META>>>
    { ...compact JSON, meta fields only, no multi-line strings... }
    <<<ALGOLIFT_FIELD:statement_md>>>
    ...raw markdown, verbatim, no escaping...
    <<<ALGOLIFT_FIELD:constraints_md>>>
    ...
    <<<ALGOLIFT_FIELD:reference_solution_py>>>
    ...raw python, verbatim...
    <<<ALGOLIFT_FIELD:brute_force_py>>>
    ...
    <<<ALGOLIFT_FIELD:input_generator_py>>>
    ...
    <<<ALGOLIFT_FIELD:checker_py>>>            (optional)
    ...
    <<<ALGOLIFT_FIELD:mutants_py[0]>>>
    ...
    <<<ALGOLIFT_FIELD:mutants_py[1]>>>
    ...
    <<<ALGOLIFT_FIELD:mutants_py[2]>>>
    ...
    <<<ALGOLIFT_FIELD:hints.l1_orientation>>>
    ...
    <<<ALGOLIFT_FIELD:hints.l2_conceptual>>>
    <<<ALGOLIFT_FIELD:hints.l3_structural>>>
    <<<ALGOLIFT_FIELD:hints.outline>>>
    <<<ALGOLIFT_FIELD:hints.editorial_md>>>
    <<<ALGOLIFT_END>>>
...
## Parsing guarantees (read before changing this file)
  ...
  - **Field order is irrelevant.** Every block is found by scanning for its delimiter line; blocks
    may appear in any order and `<<<ALGOLIFT_META>>>` need not be first.
  - **Leading/trailing prose is tolerated.** Anything before the first delimiter or after
    `<<<ALGOLIFT_END>>>` is ignored — a model that adds "Here is the problem:" before the envelope
  ...
  - **A delimiter only counts if the ENTIRE line is exactly the delimiter text** (no leading
    ...
    a deliberate, documented trade-off: `print("<<<ALGOLIFT_FIELD:x>>>")` is safe (extra characters
    ...
    byte-identical to `<<<ALGOLIFT_FIELD:...>>>` is effectively nil, versus the near-certainty of
...
META_DELIM = "<<<ALGOLIFT_META>>>"
END_DELIM = "<<<ALGOLIFT_END>>>"
FIELD_DELIM_PREFIX = "<<<ALGOLIFT_FIELD:"
FIELD_DELIM_SUFFIX = ">>>"

#: Required top-level keys of the `<<<ALGOLIFT_META>>>` JSON object. Deliberately excludes
#: `problem_id`/`version`/`state`/`hidden_tests` (server-assigned/defaulted, see module docstring)
#: and every multi-line field (those are `<<<ALGOLIFT_FIELD:...>>>` blocks instead).
...
#: Required scalar (non-indexed, non-dotted) `<<<ALGOLIFT_FIELD:...>>>` blocks.
...
def parse_envelope(text: str) -> dict[str, Any]:
    """Parses one model response in the ALGOLIFT envelope format into a plain dict shaped exactly
    like `algolift_content.models.ProblemVersion`'s constructor arguments (minus
    ...
    if duplicate_fields:
        raise EnvelopeError(
            "duplicate <<<ALGOLIFT_FIELD:...>>> block(s) — each field name must appear at most "
    ...
    if unknown_meta:
        issues.append(
            ...
            f"<<<ALGOLIFT_FIELD:...>>> block instead): {', '.join(unknown_meta)}"
        )
    if unknown_field_names:
        issues.append(
            "unknown <<<ALGOLIFT_FIELD:...>>> block name(s) (not part of the envelope schema): "
            + ", ".join(sorted(set(unknown_field_names)))
        )
    if missing_scalars:
        issues.append(
            "missing required <<<ALGOLIFT_FIELD:...>>> block(s): "
            + ", ".join(f"<<<ALGOLIFT_FIELD:{name}>>>" for name in missing_scalars)
        )
    if missing_hints:
        issues.append(
            "missing required <<<ALGOLIFT_FIELD:hints.*>>> block(s): "
            + ", ".join(f"<<<ALGOLIFT_FIELD:{name}>>>" for name in missing_hints)
        )
...
def render_envelope(data: dict[str, Any]) -> str:
    ...
    the same keys) as ALGOLIFT envelope text. Used by `StubInvoker` (so the stub genuinely

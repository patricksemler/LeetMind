...
`docker run` flag list, `@algolift/sandbox` ships a CLI and this module shells out to it:
...
**Python must never build `docker run` arguments itself** — that is `@algolift/sandbox`'s job
...
package at the time of writing: `@algolift/shared`'s `createLogger` writes pino JSON log lines to
...
def _find_repo_root(settings: Settings | None = None) -> Path | None:
    """Resolves the repo root from `ALGOLIFT_REPO_ROOT`, else by walking up from this file for
    `pnpm-workspace.yaml` (CONTRACTS.md §6.1)."""
    s = settings or get_settings()
    if s.ALGOLIFT_REPO_ROOT:
        return Path(s.ALGOLIFT_REPO_ROOT)
...
def sandbox_probe() -> tuple[bool, str]:
    ...
    if repo_root is None:
        return False, "could not locate repo root (set ALGOLIFT_REPO_ROOT, or run inside the AlgoLift workspace)"
...
def _parse_last_json_object(stdout: str) -> dict[str, Any] | None:
    ...
    path (success or `failInfra`). In practice, at the time of writing, `@algolift/shared`'s
...
def run_raw(request: SandboxRequest, *, settings: Settings | None = None) -> SandboxResult:
    ...
    if repo_root is None:
        raise SandboxUnavailable(
            "could not locate repo root (set ALGOLIFT_REPO_ROOT, or run inside the AlgoLift workspace)"
        )

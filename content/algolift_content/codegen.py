...
the sandbox via `algolift_content.sandbox.run_raw` — never in this process.
"""
...
from algolift_content.sandbox import SandboxLimits, SandboxRequest, SandboxUnavailable, run_raw
...
def render_generator_module(input_generator_py: str) -> str:
    ...
    if not _GENERATE_DEF_RE.search(text):
        raise GeneratorContractError(
            ...
            "(see the generate(rng) contract documented on algolift_content.codegen.seeded_inputs)"
        )
    return text
...
def _algolift_generate_cases():
    import json as _algolift_json
    import random as _algolift_random
    import sys as _algolift_sys
    ...
    for _i in range(count):
        seed = seed_start + _i
        rng = _algolift_random.Random(seed)
        args = generate(rng)
        cases.append({{"seed": seed, "args": args}})
    _algolift_sys.stdout.write(_algolift_json.dumps({{"cases": cases}}))


if __name__ == "__main__":
    _algolift_generate_cases()
"""
...
def seeded_inputs(
    ...
    *,
    ...
    request = SandboxRequest(
        ...
        # exec`) while implementing algolift_content.verification, which is the first caller

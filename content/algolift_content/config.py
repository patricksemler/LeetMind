...
class Settings(BaseSettings):
    ...
    # --- core / shared -----------------------------------------------------
    DATABASE_URL: str = "postgres://algolift:algolift@localhost:5432/algolift"
    PGPOOL_MAX: int = 10
    LOG_LEVEL: str = "info"
    ...
    # --- sandbox (@algolift/sandbox, used by judge/content) -----------------
    SANDBOX_PYTHON_IMAGE: str = "algolift/runner-python:1"
    SANDBOX_CPP_IMAGE: str = "algolift/runner-cpp:1"
    SANDBOX_MEMORY_MB: int = 256
    SANDBOX_CPUS: float = 1.0
    SANDBOX_PIDS_LIMIT: int = 64
    SANDBOX_WALL_TIMEOUT_MS: int = 10_000
    SANDBOX_OUTPUT_LIMIT_BYTES: int = 65_536
    SANDBOX_WORK_DIR: str = "/tmp/algolift-sandbox"
    DOCKER_BIN: str = "docker"

    # --- queue (@algolift/queue, used by judge/content) ----------------------
    ...
    # --- repo location --------------------------------------------------------
    ALGOLIFT_REPO_ROOT: str | None = None
...
def _format_validation_error(exc: ValidationError) -> str:
    ...
    header = "Invalid or missing environment variables for algolift_content.config.Settings:"
    return header + "\n" + "\n".join(lines)

...
handler in `content/algolift_content/workers/content_worker.py`'s `HANDLERS` dict:

    from algolift_content.verification import handle_verify
    HANDLERS["verify"] = handle_verify

Execution of reference/brute-force/mutant code always goes through `algolift_content.sandbox`
(never imported/exec'd in-process — PLAN.md §3). `algolift_content.codegen` supplies source
...
from algolift_content.verification.worker import handle_verify

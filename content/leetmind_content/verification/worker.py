...
be registered into `content/leetmind_content/workers/content_worker.py`'s `HANDLERS` dict:
...
returns normally (causing `leetmind_content.queue.run_worker` to `ack()` the job) whenever
...
infrastructure problems: `leetmind_content.sandbox.SandboxUnavailable` (docker/node down, the
...
from leetmind_content.models import VerifyJobPayload
